...
class Client(metaclass=ClientVerifier):
    ...
    @Log()
    def message_from_server(self, s, username):
        while True:
            try:
                data = s.recv(1000000)
    ...
    @Log()
    def user_command(self, sock, username):
        ...
        while True:
            ...
            if command == 'message':
                ...
                msg_jim = json.dumps(self.create_exit_message(username))
    ...
    def main(self):
        ...
        data_msg = self.create_msg_for_server(msg, client_name)
        ...
        receiver = threading.Thread(target=self.message_from_server, args=(s, client_name))
        receiver.daemon = True
        receiver.start()

        user_interface = threading.Thread(target=self.user_command, args=(s, client_name))
...
def main():
    ...
    client_new = Client(serv_addr, serv_port)
    client_new.main()
